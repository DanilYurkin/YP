...
def create_tablo(index):
    ...
    if index:
        print(f'создаем табло\n{index}')
        for i in range(len(word)):
            if list(letters_dictionary.values())[i][1] == '1':
    ...
    def enter_click(self):
        ...
        if len(answer) > 1:
            ...
            self.log_box.insertPlainText(f'Буква "{answer}"!\n'
                                         f'Количество букв в слове - {c}\n'
                                         f'Расположение: {letter_position}\n')
            # print(letters_dictionary)
            # print(letter_position)
            # print('=' * 40)
            # self.tablo.addWidget(create_tablo(letters_dictionary))

        for i in reversed(range(self.tablo.count())):
            item = self.tablo.itemAt(i)
            self.tablo.removeItem(item)
        self.tablo.addWidget(create_tablo(letters_dictionary))
